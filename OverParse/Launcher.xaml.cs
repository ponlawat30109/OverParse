...
namespace OverParse
{
    ...
    public partial class Launcher : Window
    {
        ...
        private void CheckDll()
        {
            ...
            if (pso2h == "7a616bf1f82bbc366cffd813a3c41f4e9dda060319af1b169bad37bcf48f0899") { HookResult.Content = "OK (4.0.0)"; HookResult.Foreground = okbrush; }
            else if (pso2h == "601d0b4e8d477fec3c2bab061ab4555af33f7c248d5121fe9211f7103d91a27b") { HookResult.Content = "OK (3.1.7)"; HookResult.Foreground = okbrush; }
            else if (pso2h == "aaee1145b0bfc202f1a4fbf3b414d8d83a731d2d8d2544735efe6a17cb657fc0") { HookResult.Content = "ERROR (3.1.6)"; HookResult.Foreground = errorbrush; } 
            else { HookResult.Content = "Unknown"; HookResult.Foreground = passbrush; }

            if (ddraw == "f462107109553bd0fe271978e1462e288e76e12c2b3238ebc0efa66655290447") { DDrawResult.Content = "OK (4.0.0)"; DDrawResult.Foreground = okbrush; }
            else if (ddraw == "0e2700bba52170f9dc5b4c811a7d7f23e22e9b39eae30006e1947a4acf05f63d") { DDrawResult.Content = "OK (3.1.7)"; DDrawResult.Foreground = okbrush; } 
            else if (ddraw == "cb0d66924d2e5f98cc5bb7c59c353b248f7460822f5d6be8c649ef9aecce48b3") { DDrawResult.Content = "ERROR (3.1.6)"; DDrawResult.Foreground = errorbrush; } 
            else { DDrawResult.Content = "Unknown"; DDrawResult.Foreground = passbrush; }

            if (dmgdump == "46d396a3f1c08c2b4b9ea39d4cb56f989fbaca875d06ca607784a77b6771905f") { DmgdmpResult.Content = "OK (4.0.0)"; DmgdmpResult.Foreground = okbrush; }
            else if (dmgdump == "33ddffb54c4267ea62cc4ecf50ea12f19be9304d196e164439cc7091b6d55a41") { DmgdmpResult.Content = "OK (3.1.7)"; DmgdmpResult.Foreground = okbrush; } 
            else if (dmgdump == "82bb5d1f9e04827947dc025068b8593f0e5c8e1ec5e4624233b5bcb4e7de3f18") { DmgdmpResult.Content = "ERROR (3.1.6)"; DmgdmpResult.Foreground = errorbrush; } 
            else { DmgdmpResult.Content = "Unknown"; DmgdmpResult.Foreground = passbrush; }

            if (File.Exists(Properties.Settings.Default.Path + "\\plugins\\PSO2DamageDump.cfg")) { DmgcfgResult.Content = "OK"; DmgcfgResult.Foreground = okbrush; }
            else { DmgcfgResult.Content = "Not Found"; DmgcfgResult.Foreground = errorbrush; }
            ...
            {
                Status.Content = "Dll Install Failed";
            }
        }
        ...
        {
            if (!Properties.Settings.Default.BanChecked || !IsInstalled()) { Application.Current.Shutdown(); }
